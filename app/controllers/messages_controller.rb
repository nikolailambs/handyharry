class MessagesController < ApplicationController
  ...
  def create
    ...
    if @message.content.include? "@"
      ...
      project = Project.where(title: task_array[index_project].gsub(/\W+/, '') )#.where(handy_id: @chat_room.handy_id).where(client_id: @chat_room.client_id)

      task_array = task_array.slice((index_project+1)..(task_array.length-1))
      task = task_array.join(" ")
      new_task = Task.new(title: task)
